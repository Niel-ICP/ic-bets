...
actor ICBetting {
  ...
  let ref = Principal.fromText("2vxsx-fae");

  var bets: Buffer.Buffer<Bet> = Buffer.fromArray([]);
  stable var betsEntries: [Bet] = [];
  stable var currentBet: Nat = 0;
  ...
  Map.set(referees, phash, ref, true);
  ...
  public shared(msg) func placeBet(bet: Nat, option: Nat, amount: Nat): async Text {
    ...
    _bet := {
      name = _bet.name;
      totalBets = _bet.totalBets + amount;
  ...
  public query func calculateWinnings(user: Principal, bet: Nat): async Nat {
    ...
    var _winnings: Nat = 0;
    if(_bet.winner == 0) {
        _winnings := _uB.amountBetted * _bet.totalBets / _bet.betsOnA;
    };
    if(_bet.winner == 1) {
        _winnings := _uB.amountBetted * _bet.totalBets / _bet.betsOnB;
    };
    return _winnings;
  };
  ...
  public query func getBetsSize() : async Text {
    return Nat.toText(bets.size());
  };
  ...
  public shared(msg) func closeBets(bet:Nat): async Text {
    ...
    if(_isReferee != true) {
      return "Only a referee can close the bets";
    };
    var _bet: Bet = bets.get(bet);
    if(_bet.betsClosed) {
      return "Bets are already closed";
  ...
  system func preupgrade() {
      betsEntries := bets.toArray();
  };

  system func postupgrade(){
      bets:= Buffer.fromArray(betsEntries);
  };
};
